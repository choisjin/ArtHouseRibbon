...
결과는 data/world/render/ 에
  <방>.png        TV 배경
  <방>_env.hdr    리본이 조명용 360° 환경
  <방>.json       렌더에 쓴 배치(TV 가 가림막·길찾기에 같은 배치를 쓴다) + 시각
...
SCRIPT = REPO / "tools" / "blender" / "render_room.py"
...
class WorldRenderer:
    ...
    def info(self, room: str) -> Optional[Dict[str, Any]]:
        """렌더가 있으면 TV 가 쓸 주소와 그때의 배치"""
        meta = _read_json(self.dir / f"{room}.json")
        png = self.dir / f"{room}.png"
        if not meta or not png.exists():
            return None
        v = int(meta.get("rendered_at", 0))
        env = self.dir / f"{room}_env.hdr"
        return {
            "bg": f"/world-render/{room}.png?v={v}",
            "env": f"/world-render/{room}_env.hdr?v={v}" if env.exists() else None,
    ...
    async def _render(self, room: str) -> bool:
        ...
        cmd = [self.blender, "-b", "--factory-startup", "-P", str(SCRIPT), "--",
               room, str(lay_file), str(self.world.art_dir.parent), str(work)]
        env = dict(os.environ, PYTHONUNBUFFERED="1", RENDER_PCT=str(self.pct), RENDER_SAMPLES=str(self.samples))
        log.info("배경 렌더 시작: %s (%s%%, %s samples)", room, self.pct, self.samples)
        ...
        if rc != 0 or not png.exists():
            ...
            return False
        os.replace(png, self.dir / png.name)
        if hdr.exists():
            os.replace(hdr, self.dir / hdr.name)
        _write_json(self.dir / f"{room}.json", {"room": room, "layout": layout, "rendered_at": int(time.time())})
        shutil.rmtree(work, ignore_errors=True)
