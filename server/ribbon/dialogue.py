...
class DialogueManager:
    ...
    async def cancel_listening(self) -> None:
        """듣는 중인데 아직 말이 없을 때 버튼을 한 번 더 눌렀다 (2026-09-21): **말없이** 이번 입력만 취소한다"""
        self.queue.clear()
        await self.broadcast({"type": "speak.stop"})
    ...
    def _idle_listening(self) -> bool:
        """버튼으로 듣기 시작했는데 아직 아무도 말하지 않은 상태인가 (차례 없음, 말하거나 생각하는 중도 아님)"""
        return (self.ribbon_state == "listening" and self.queue.active() is None and not self.busy()
    ...
    async def heard_nothing(self, channel: int) -> None:
        """말 조각을 받았지만 인식된 글자가 없다 (잡음·너무 짧음). 그 아이의 빈 차례면 말없이 접는다"""
        active = self.queue.active()
        if active is not None and active.channel == channel and not active.text and not self._responding:
            log.info("인식된 말이 없어 차례를 접음 (ch=%d)", channel)
