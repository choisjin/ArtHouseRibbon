...
**TV 카메라 (웹캠)**: TV 노트북에 웹캠을 꽂고 TV 위 가운데에 아이들 쪽을 향하게 단다. 위 플래그 설정이 필요하다 (http 주소에서는 카메라도 막힌다).
관리자 페이지 → 설정 → TV 카메라에서 장치를 고르고 "카메라 켜기" → TV 노트북에서 카메라 권한 허용 (처음 한 번).
"📺 TV에 미리보기" 를 누르면 TV 구석에 20초 동안 카메라 화면과 얼굴 상자가 떠서 방향을 맞출 수 있다. 켜 둔 상태는 그 브라우저가 기억한다.
