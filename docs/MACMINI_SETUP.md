...
**브라우저는 Chrome 을 쓴다** (TV·마이크·편집기 모두). Safari 는 소리 출력 장치 선택과 자동 재생 허용 옵션이 없고,
...
마이크 소리는 **관리자 페이지가 서버로 보낸다.** 무선 마이크 수신기가 꽂힌 컴퓨터에서 관리자 페이지를 열어 두어야 리본이가 듣는다.

- 맥미니에 꽂혀 있으면: `http://localhost:8765/?mode=admin` (`start_ribbon.command` 가 자동으로 연다)
- TV 노트북에 꽂혀 있으면: TV 노트북에서 관리자 페이지를 하나 더 열어 둔다

처음 한 번 **설정 → 마이크**에서 "마이크 장치 찾기" → 1세트(마이크 1·2)·2세트(마이크 3·4) 장치를 고르고 "마이크 켜기".
그 브라우저가 기억해 두었다가 다음부터는 열자마자 켠다 (끄면 다음에 켜지지 않는다). 다른 탭으로 옮겨도 계속 받고, 위 막대에 "🎙 마이크 켜짐" 이 보인다.
"잠김: 화면을 한 번 누르세요" 가 뜨면 그 창을 한 번 클릭한다. 같은 컴퓨터에 관리자 창이 여럿이어도 마이크는 한 창만 받는다. 서버 창에는 `마이크 소리 들어옴: 채널 N` 이 찍힌다.
...
리본이 목소리가 마이크(특히 웹캠 마이크)로 다시 들어가 새 질문이 되지 않도록, 리본이가 말하는 동안과 끝난 뒤 0.7초는 마이크 소리를 버린다
(`RIBBON_ECHO_GUARD`, `RIBBON_ECHO_TAIL_MS`). 그래서 리본이가 말하는 중에는 다른 아이의 호출도 듣지 못한다.
말이 끝난 직후 리본이가 제 말꼬리를 듣는 것 같으면 `RIBBON_ECHO_TAIL_MS` 를 1200 쯤으로 늘린다.

### 웹캠

웹캠도 마이크처럼 **맥미니의 관리자 페이지가 받는다.** 맥미니에 USB 웹캠을 꽂고 TV 위 가운데에 아이들 쪽을 향하게 단다.
처음 한 번 **설정 → 카메라**에서 "카메라 장치 찾기" → 웹캠을 고르고 "카메라 켜기" (브라우저가 권한을 물으면 허용.
맥이 막으면 시스템 설정 → 개인정보 보호 및 보안 → 카메라에서 Chrome 허용). 켜져 있으면 설정 탭에 카메라 화면과 얼굴 상자가 보이니 그것을 보며 방향을 맞춘다.
그 브라우저가 기억해 두었다가 다음부터는 열자마자 켠다. 영상은 저장하거나 보내지 않고 얼굴 위치만 TV 로 간다.
아이들이 테이블에 앉은 거리(2m 안쪽)에서 잘 찾는다. 더 멀면 얼굴을 놓칠 수 있다.
...
관리자 페이지 → 설정 → TV 소리 출력에서 실제 스피커/HDMI 로 고르고 "삐 소리 시험" (장치 이름이 안 보이면 "장치 이름 보기", TV 컴퓨터에서 권한 허용).
