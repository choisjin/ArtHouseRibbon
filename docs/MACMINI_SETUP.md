...
- TV 노트북에 꽂혀 있으면: TV 주소를 `?mode=tv&mic=1` 로 (TV 화면에는 마이크 칸을 띄우지 않는다)

장치 고르기·켜기·끄기는 **관리자 페이지 → 설정 → 마이크**에서 한다 (휴대폰으로도 된다). 마이크를 받는 화면마다
1세트(마이크 1·2)·2세트(마이크 3·4) 장치를 고르고 "마이크 켜기". 그 화면의 브라우저가 기억해 두었다가 다음부터는 열자마자 켠다.
"켜졌지만 잠김" 이 뜨면 그 컴퓨터의 마이크 화면을 한 번 클릭한다. 서버 창에는 `마이크 소리 들어옴: 채널 N` 이 찍힌다.
