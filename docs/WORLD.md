...
| Peek | 화면(유리) 코앞에 붙어 두 손을 짚고 들여다볼 때 |
...
가끔(90초에 한 번 이상 간격) 방 앞쪽 가운데로 걸어와 **길찾기 격자 밖으로 한 걸음 더**(`GLASS_STEP`) 나가서
우리 쪽으로 돌아 두 손을 올리고 5초쯤 들여다본다. 다 보면 다시 격자 안으로 돌아온다
(격자 밖에 서 있으면 다음 길을 못 찾는다). 부르면 바로 물러나 대화로 넘어간다.
더 나가면 커 보이지만 발이 화면 아래로 잘린다 — TV 카메라가 눈높이에서 수평으로 보고 있어서다.
