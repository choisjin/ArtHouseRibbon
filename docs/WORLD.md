...
친구는 대화하지 않고 돌아다니기만 합니다. 두 캐릭터는 길을 찾을 때 서로의 자리와 가는 곳을 피해 가고,
걷다가 부딪힐 것 같으면 멈춥니다. 리본이는 잠깐 기다렸다가 돌아가는 길을 다시 찾고, 친구는 리본이가 지나갈 길 밖으로 비켜섭니다
(리본이가 부르면 오는 자리에 친구가 서 있어도 비켜섭니다). 캐릭터나 친구를 바꾸면 모델을 새로 읽어야 해서 TV 화면이 한 번 새로 열립니다.
...
### 화면에 바짝 붙어 들여다보기 (유리창 층, `client/src/tv/glass.ts`)

가끔(90초에 한 번 이상 간격) 방 앞쪽으로 걸어와 **화면 아래로 걸어 나간다**. 그러면 같은 캐릭터가 화면 아래에서 크게 올라와
유리(TV 화면)에 손을 짚고 얼굴을 붙인 채 너머를 보려고 애쓴다: 왼쪽으로 옮겨 가며 두리번 → 오른쪽 → 까치발로 위를 기웃 → 가끔 입김으로 유리가 뿌옇게.
6초쯤 뒤 스르르 내려가고 방 캐릭터가 다시 걸어 들어온다. 같이 나온 친구도 가끔(70%) 옆에 나란히 붙는다. 부르면 바로 내려가 방으로 들어온 뒤 대화하러 온다.

방 캐릭터를 앞으로 걸어오게만 해서는 안 되는 이유: TV 카메라가 방 앞면에서 8m 뒤, 2.3m 높이에서 수평으로 보고 있어서
화면 아래 끝이 곧 방 앞면 바닥이다. 앞으로 나오면 크게 보이기 전에 화면 아래로 빠져나간다 (배경이 블렌더 렌더라 카메라도 못 움직인다).
그래서 붙어 있는 순간은 방 위에 겹쳐 그리는 별도 층(자기 카메라·조명, 방과 같은 환경맵)에 사본을 그린다. 입김은 화면 위 DOM 한 겹 (`.glass-fog`).
...
앉을 때는 몸만 좌판 높이로 올리고 그림자는 바닥에 남긴다. 의자 옆(또는 앞)에 섰다가 좌판 쪽으로 옮겨 가며 **천천히 몸을 올려 앉고**,
일어날 때는 좌판에서 먼저 빠져나온 뒤 천천히 내려와 그 자리에 선다 (약 1초. 서서 좌판까지 걸어 들어가면 다리가 의자·책상을 뚫는다).
뛰는 동작은 쓰지 않는다: 아이들이 의자에서 뛰는 걸 따라 할 수 있다. 부르면 먼저 내려온 뒤 온다.
책상 앞에 나란히 붙인 가운데 의자(앞은 책상, 양옆은 다른 의자)는 뒤에서 조용히 들어간다. 들어가고 나오는 동안 잠깐 등받이와 겹쳐 보인다 (의자는 배경 렌더라 빼낼 수 없다).

**가구와 겹치지 않기**: 길찾기 격자는 캐릭터 몸 반지름만큼 가구를 부풀려 막는다. 반지름은 모델 꼭짓점 중 몸 가운데 축에서 먼 거리의 99% 지점
(큰 머리·양갈래·리본까지 들어간다, `ribbon3d.ts` horizontalRadius). 두 캐릭터가 같은 격자를 쓰므로 더 큰 쪽에 맞춘다.
칸 가운데만 보고 막으므로 반 칸만큼 더 부풀린다 (`nav.ts`). TV 주소에 `&probe=1` 을 붙이면 `window.__probe` 로 캐릭터·길찾기를 볼 수 있다 (자동 검사용).
부르면 먼저 일어선다.
