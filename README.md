...
- 아이: 추가·수정·삭제, 이름·나이·마이크 채널 (TV 에는 그리지 않음)
- 리본이: 이름, 목소리(Supertonic 10종) 미리 듣기, 속도·품질, 한 번에 말할 문장 수, 성격 추가 지시문,
  겉모습(원피스/투피스, 머리·리본·옷 색, 3D 미리보기), 돌아다니기 켜기/끄기·걷는 속도·대화 뒤 다시 돌아다니기까지 시간
- 맵: TV 에 보여줄 방(미술실/전시장) 선택, 맵 편집기 열기
...
- 전시실 꾸미기 `?mode=art&kid=<아이 id>`: 아이마다 있는 전시실. 작품 사진을 올려(배경 지우기) 벽에 걸고, 끌어서 옮기고 크기를 바꿉니다(비율 고정). 관리자 페이지에서 아이를 고르면 버튼이 있습니다 (`docs/WORLD.md`)
- 저장하면 `data/kids.json`, `data/settings.json`, `data/world/` 에 기록되고 TV 화면에 즉시 반영된다. 서버 재시작 불필요.
