...
- **설정**: 마이크(마이크 화면·TV 의 장치 고르기·켜기·끄기·음량), TV 소리 출력 장치·삐 소리 시험, 화면 스타일 라이트 / 다크 / 기기 설정 따르기
- 마이크 화면 `?mode=mic`: 무선 마이크 수신기가 꽂힌 컴퓨터에서 켜 두면 소리가 서버로 간다 (TV 노트북이면 `?mode=tv&mic=1`).
  장치 고르기·켜기는 관리자 '설정' 탭에서 (그 화면이 알려 온 장치 목록을 보고 서버를 거쳐 조작한다). TV 소리 출력 장치도 같은 탭
