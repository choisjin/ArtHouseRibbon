...
- 3D 미술실에서 리본이가 돌아다니고 (아이 네 명은 화면에 나오지 않고 대기열에만 등록)
...
1. 호출 + 대기열 대화 (이 골격) ← 지금
2. 3D 방·리본이 (Character_Creator 블렌더 모델, `docs/WORLD.md`) ← 완료. TV 위 폰 얼굴 추적 → 리본이 시선 (MediaPipe)
...
TV·마이크·관리자·편집기 화면 모두 **Chrome** 을 쓴다. Safari 는 소리 출력 장치 선택과 자동 재생 옵션이 없어 소리가 안 나는 경우가 있다.

## 관리자 페이지

`http://<서버>:8765/?mode=admin`

휴대폰으로도 쓸 수 있다 (좁은 화면에서는 탭이 아래 막대로 가고, 시간표는 손가락으로 끌어 옮긴다).

- **대시보드**
  - 캐릭터 조작: 중단·모두 멈춤·호출 무시, 대기열, **TV 에 보여줄 방**, 지금 수업 아이들(등원·하원, 마이크 1~4 지정, 호출), 대화 기록
    - 단축키(키보드): `1`~`4` 그 마이크 아이 호출 · `Esc` 중단(다음 차례로) · `Shift+Esc` 모두 멈춤 · `M` 호출 무시 켜기/끄기 · `P` 화면에 바짝 붙어 보기(연출 시험)
  - 수업 시간표: 월~금 15:00~19:00, 일·주·월 보기. 아이 이름을 끌어 옮기면 그날만 바뀜
- **아이들**: 추가·수정·삭제. 이름·별명·생일·등원 시작일·기본 마이크, 매주 정규 수업 시간, 좋아하는 것·선생님 메모 (대화에 쓰임)
- **캐릭터**: TV 에 나올 주인공·친구, 캐릭터별 프로필 카드. 설정 페이지에서 이름·소개·성격·목소리(미리 듣기)·옷.
  대화·움직임 공통 설정(문장 수, 즉시 반응·추임새, 돌아다니기)
- **맵**: 맵 편집기. 편집기의 방 목록에서 아이 전시실도 고른다 (고르면 전시실 꾸미기로 바뀜)
- **설정**: 마이크(이 컴퓨터에서 받기: 장치 고르기·켜기·끄기·음량), TV 소리 출력 장치·삐 소리 시험(TV 로 원격 전달), 화면 스타일 라이트 / 다크 / 기기 설정 따르기
- 마이크: 무선 마이크 수신기가 꽂힌 컴퓨터(맥미니)에서 **관리자 페이지를 열어 두면** 그 페이지가 마이크 소리를 서버로 보낸다.
  설정 탭에서 장치를 고르고 켜면 그 브라우저가 기억해서 다음부터 열자마자 켠다. 창을 여러 개 열어도 마이크는 한 창만 받는다.
  (예전 마이크 화면 `?mode=mic` 는 관리자 설정 탭으로 넘어간다. TV 의 `?mic=1` 은 없앰)
- 맵 편집기 `?mode=editor`: 가구 배치, 그림 올리기·걸기, 리본이 "부르면 오는 자리". `data/world/<방>.json` 에 저장 (`docs/WORLD.md`)
- 전시실 꾸미기 `?mode=art&kid=<아이 id>`: 아이마다 있는 전시실. 작품 사진을 올려(배경 지우기) 벽에 걸고, 끌어서 옮기고 크기를 바꿉니다(비율 고정). 맵 편집기의 방 목록(관리자 '맵' 탭)에서 고릅니다 (`docs/WORLD.md`)
- 저장하면 `data/kids.json`, `data/settings.json`, `data/schedule.json`(시간표의 하루짜리 변경), `data/world/` 에 기록되고 TV 화면에 즉시 반영된다. 서버 재시작 불필요.
