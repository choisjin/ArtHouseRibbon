...
- **대시보드**: 수업 시간표(월~금 15:00~19:00, 일·주·월 보기. 아이 이름을 끌어 옮기면 그날만 바뀜),
  지금 수업 아이들(등원·하원, 마이크 1~4 지정, 호출), 캐릭터 조작과 단축키, 대화 기록
  - 단축키: `1`~`4` 그 마이크 아이 호출 · `Esc` 중단(다음 차례로) · `Shift+Esc` 모두 멈춤 · `M` 호출 무시 켜기/끄기
...
- **맵**: 맵 편집기를 그대로 넣었고, 위에서 방 또는 아이 전시실을 고른다. TV 에 보여줄 방 선택
...
- 전시실 꾸미기 `?mode=art&kid=<아이 id>`: 아이마다 있는 전시실. 작품 사진을 올려(배경 지우기) 벽에 걸고, 끌어서 옮기고 크기를 바꿉니다(비율 고정). 관리자 페이지 '맵' 탭에서 고릅니다 (`docs/WORLD.md`)
