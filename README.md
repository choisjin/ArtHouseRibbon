...
1. 호출 + 대기열 대화 (이 골격) ← 지금
2. 픽셀아트 방·캐릭터 (PixelLab 생성, `docs/ASSETS.md`) ← 완료. TV 위 폰 얼굴 추적 → 리본이 시선 (MediaPipe)
...
- 방 꾸미기: 가구 PNG 를 드래그해 배치, 앵커(자리·문·리본이) 편집, 새 가구 PNG 추가. `data/room.json` 에 저장.
- 저장하면 `data/kids.json`, `data/settings.json`, `data/room.json` 에 기록되고 TV 화면에 즉시 반영된다. 서버 재시작 불필요.
