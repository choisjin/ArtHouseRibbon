...
export class RibbonBrain {
  ...
  /** 의자에서 올라왔던 자리로 폴짝 내려온다 (그대로 걸어 나가면 의자·책상을 뚫는다) */
  ...
  update(dt: number): void {
    ...
    switch (m.kind) {
      ...
      case "peek":
        ...
        break;
      case "walk":
        if (!this.body.moving && !this.body.hopping) this.mode = { kind: "idle", until: now + 2 + Math.random() * 5 };
        break;
      case "idle":
  ...
  /** 의자에 가서 앉기: 의자 앞이나 옆까지 걸어가서 좌판으로 폴짝 올라앉는다 */
