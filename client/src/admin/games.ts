 ...
 */
interface Game { mode: string; name: string; desc: string }
interface Category { id: string; name: string; desc: string; games: Game[] }

const CATEGORIES: Category[] = [
  {
    id: "pokemon", name: "포켓몬 맞추기", desc: "리본이가 내는 문제를 교실이 같이 맞혀요. 맞히면 다음 문제로 이어집니다.",
    games: [
      { mode: "describe", name: "설명 듣고 맞추기", desc: "타입·생김새 설명을 듣고 누구인지 맞혀요" },
      { mode: "image", name: "그림 보고 맞추기", desc: "그림을 보고 이름을 맞혀요" },
      { mode: "peek", name: "조금 보고 맞추기", desc: "조금만 보이는 그림을 보고 맞혀요" },
    ],
  },
];
...
export function mountGames(el: HTMLElement, ctx: AdminCtx): { show(): void } {
  el.innerHTML = CATEGORIES.map((c) => `
    <section class="card">
      <div class="row"><h2>${c.name}</h2><span class="grow"></span>
