...
/**
 * 관리자 페이지 (?mode=admin). 탭 네 개:
 *   #dashboard   수업 시간표 · 지금 수업 아이들(등원·마이크·호출) · 캐릭터 조작 단축키
...
export async function startAdmin(socket: RibbonSocket): Promise<void> {
  ...
  <div id="admin">
    <header class="topbar">
      <h1>🎀 리본 관리자</h1>
      <nav>${TABS.map((t) => `<a href="#${t.id}" data-tab="${t.id}">${t.icon} ${t.name}</a>`).join("")}</nav>
  ...
  function mount(id: TabId): void {
    ...
    mounted.add(id);
    const el = panel(id);
    if (id === "dashboard") mountDashboard(el, ctx, () => active === "dashboard");
    if (id === "kids") kidsTab = mountKids(el, ctx);
    if (id === "characters") characters = mountCharacters(el, ctx);
    if (id === "map") mapTab = mountMap(el, ctx);
  }
