...
- **전시실 9차 손질 — 작품에 붙는 것은 편집 창으로** (2026-09-21 요청 묶음):
  · 아래 설정 창은 **작품 · 핀 조명 · 조명** 세 탭. '하이라이트' 를 **핀 조명**으로 바꾸고 **걸린 작품을 골랐을 때만** 보인다.
  · **필터와 크기는 작품 편집 창으로** 옮겼다. 편집 창은 **전체화면**이고 다섯 탭: **크기**(벽에 걸 가로 + 네 쪽 여백) ·
    배경 · 액자 · **필터**(골라 쓰기 + 밝기·대비·채도·색온도·세피아) · 설명. 미리 보기가 빈 자리를 가득 채운다.
  · 그래서 **크기(size)와 필터(fx)도 작품에 붙는다** (`world_store.art_look`): 걸 때 그 값으로 걸리고, 편집하면 걸려 있는 곳이
