...
- 마이크는 `?mode=mic` 화면이 보낸다 (장치 기억·자동 켜기). TV 에서 받으려면 `?mode=tv&mic=1`. 장치 고르기·켜기와 TV 소리 출력은 관리자 '설정' 탭 (device.status/control, `server/ribbon/devices.py`). start_ribbon 이 맥미니에서 마이크 화면을 연다.
