...
- **포켓몬 맞추기 게임** (`server/ribbon/games/pokemon_quiz.py`, TV `client/src/tv/game.ts`): 설명 듣고 / 그림 보고 /
  가린 그림(6x6 중 4칸부터, 힌트마다 흩어진 5칸씩). 한국어 이름 기준, 인식이 조금 틀려도 정답. **힌트는 달라고 할 때만**
...
- **입력 방식** `input_mode` (기본 **button**, 사용자 요청 2026-09-20 "모든 입력은 버튼을 누른 후에, 대기하지 마"):
  ...
  버튼을 누르면 리본이가 바로 **"지금 말해줘."** (`persona.BUTTON_PROMPT`) -> 끝난 뒤부터 `button_window_s` 동안 듣는다.
  이 말과 추임새는 서버가 켜질 때·목소리를 바꿀 때 미리 합성해 둔다 (`dialogue.prewarm`, `_tts_cache`).
