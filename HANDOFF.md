...
- **호출 버튼: 한 번 누르면 시작, 한 번 더 누르면 마무리** (2026-09-23 요청, `main._on_button`):
  대화 중(말하거나 생각하는 중, 듣는 중, 이어 말하기 기다리는 중, 게임 중 = `dialogue.in_conversation()`)에 누르면
  모두 멈추고 "응, 그럼 다음에 또 이야기하자!" 하고 끝낸다 (`dialogue.end_by_button`). 그 밖에는 새로 듣는다.
  (2026-09-21 의 "이어서 말할까?" 묻기·말없이 취소는 없앴다.)
